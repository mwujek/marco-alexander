...
class Dynamic_Styles {
	...
	public function setFont($element, $option_key, $important = false) {
		...
		if($this->use_options) {
			...
			$font_weight = $this->get_option($option_key, 'weight');
			$fonts = array();
			if(!empty($font) && is_string($font_family) && strpos($font, "_safe_") === false) {
				$fonts[md5($font)]['font-family'] = $font;
			}

			if ( !empty($font_weight) && is_string($font_weight)  ) {
				$fonts[md5($font_family)]['weight'] = $font_weight;
			}
	...
	protected function importGoogleFonts() {
		...
		foreach($this->google_fonts as $font) {
			if(!empty($font)){
				$family = ( array_key_exists( 'font-family', $font ) )? $font['font-family']: '';
				if ( !empty( $family ) ) {
					echo "@import url(//fonts.googleapis.com/css?family=" . $family . ");";
				}
	...
	protected function mergeFonts($fonts) {
		$this->google_fonts = array_merge( $this->google_fonts, $fonts );
	}
