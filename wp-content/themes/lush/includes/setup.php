...
if(!empty($_GET["import"]) && $_GET["import"] == 'revslider') {
...
function iron_get_demos() {
	...
	$themes = false;
	$themes_url = 'http://lush.irontemplates.com/import/themes.php';
...
*/
	...
	$file_content = file_get_contents('http://lush.irontemplates.com/import/default-data'.$theme.'.xml');
...
function iron_widgets_init() {
	...
	if ( function_exists('get_iron_option') ) :
		...
		$params = array(
			...
			, 'after_title'   => '</h3></div><span class="heading-side"></span>'
		);
...
function iron_enqueue_styles() {
	...
	wp_enqueue_style('iron-fancybox', IRON_PARENT_URL.'/css/fancybox.css', false, '', 'all' );
	wp_enqueue_style('font-awesome', '//netdna.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.min.css', false, '', 'all' );
	wp_enqueue_style('iron-master', IRON_CHILD_URL.'/style.css', false, '', 'all' );

	if (!empty($_SERVER['HTTP_USER_AGENT']) && preg_match('/(?i)msie [6-8]/',$_SERVER['HTTP_USER_AGENT']) )

	wp_enqueue_style('iron-msie', IRON_PARENT_URL.'/css/ie.css', array('iron-master'), '', 'all');

	$custom_styles_url = '?load=custom-style.css';
	...
	wp_enqueue_style('custom-styles', get_option('home') . $custom_styles_url , array('iron-master'), '', 'all' );

	// custom page title
	wp_enqueue_style('page-banner', IRON_PARENT_URL . '/css/page-banner.css', false, NULL, 'all' );
	if ( get_field( 'banner_parallax' ) ) {
		wp_enqueue_script( 'velocity', IRON_PARENT_URL . '/js/velocity/velocity.min.js', FALSE, NULL, TRUE );
		wp_enqueue_script( 'iron-header-parallax', IRON_PARENT_URL . '/js/iron_parallax.js', array('jquery','velocity'), NULL, TRUE );
	}
}
...
function iron_enqueue_admin_styles() {
	...
	wp_enqueue_style('iron-vc', IRON_PARENT_URL.'/admin/assets/css/vc.css', false, '', 'all' );
	wp_enqueue_style('iron-acf', IRON_PARENT_URL.'/admin/assets/css/acf.css', false, '', 'all' );
...
*/
	...
	if(get_iron_option('enable_nice_scroll')) {
		 wp_enqueue_script('gambit-smoothscroll', IRON_PARENT_URL.'/js/gambit-smoothscroll.js', null, true);
	}
...
*/
	...
	// VENDORS
	wp_enqueue_script('iron-utilities', IRON_PARENT_URL.'/js/utilities.js', array('jquery'), null, true);
	wp_enqueue_script('iron-plugins', IRON_PARENT_URL.'/js/plugins.all.min.js', array('jquery'), null, true);
	wp_enqueue_script('iron-parallax', IRON_PARENT_URL.'/js/jquery.parallax.js', array('jquery'), null, true);
	wp_enqueue_script('iron-twitter', IRON_PARENT_URL.'/js/twitter/jquery.tweet.min.js', array('jquery'), null, true);

	if(defined('ICL_LANGUAGE_CODE') && ICL_LANGUAGE_CODE != 'en') {

		wp_enqueue_script('iron-countdown-l10n', IRON_PARENT_URL.'/js/countdown-l10n/jquery.countdown-'.ICL_LANGUAGE_CODE.'.js', array('jquery'), null, true);
	}

	wp_enqueue_script('iron-main', IRON_PARENT_URL.'/js/main.js', array('jquery', 'iron-plugins'), null, true);
	...
	wp_localize_script('iron-main', 'iron_vars', array(
		...
		'header_logo_hide_on_scroll' => get_iron_option('header_logo_hide_on_scroll'),
...
function iron_enqueue_admin_scripts() {
	...
	wp_enqueue_script('iron-admin-custom', IRON_PARENT_URL.'/admin/assets/js/custom.js', array('jquery'), null, true);
	wp_enqueue_script('iron-admin-vc', IRON_PARENT_URL.'/admin/assets/js/vc.js', array('jquery'), null, true);
